...
class RobotArmEnvV3(gym.Env):
    metadata = {'render.modes': ['human']}

    def __init__(self):
        self.set_window_size([800, 800])
        self.set_link_properties([100, 100,100])
        self.set_increment_rate(0.5)
        ...
        self.observation_space = spaces.Box(np.finfo(np.float32).min, np.finfo(np.float32).max, shape=(4,),
                                            dtype=np.float32)
    ...
    def draw_arm(self, theta):
        ...
        for i in range(1, len(P)):
            F_next = base.dot(P[i])
            pygame.draw.line(self.screen, LINK_COLOR, (int(F_prev[0, 3]), int(F_prev[1, 3])),
                             (int(F_next[0, 3]), int(F_next[1, 3])), 5)
    ...
    def generate_random_angle(self):
        ...
        theta[:2] = np.random.uniform(self.min_theta, self.max_theta, size=2)
        return theta
